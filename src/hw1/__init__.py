...
train_data: np.ndarray[tuple[N_IMAGES, Literal[3], Literal[32], Literal[32]], np.dtype[np.float64]] = np.concatenate(
...
test_data: np.ndarray[tuple[N_IMAGES, Literal[3], Literal[32], Literal[32]], np.dtype[np.float64]] = (
    _test_data["data"].astype(np.float64).reshape(-1, 3, 32, 32)
)
...
def get_test_set_dataloader(
    ...
    testset = CustomCIFAR10(
        root="../../data", train=False, download=False, transform=transform, additional_features=additional_features
    )
